...
export const getAdministradorById = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const alertasResponse: QueryResult = await pool.query(alertasQuery, [id]);
...
export const cambiarPassword = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const passwordMatch = await bcrypt.compare(password_actual, adminResponse.rows[0].password);
    ...
    const response: QueryResult = await pool.query(updateQuery, [hashedNewPassword, id]);
...
export const restablecerPassword = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const updateQuery = `
      ...
      RETURNING id_administrador, usuario
    `;
    const response: QueryResult = await client.query(updateQuery, [hashedPassword, id]);
...
export const toggleAdministrador = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const estadoQuery = `
      ...
      WHERE adm.id_administrador = $1
    `;
    const estadoResponse: QueryResult = await client.query(estadoQuery, [id]);
    ...
    if (nuevoEstado === false) {
      ...
      `;
      const alertasPendientesResponse: QueryResult = await client.query(alertasPendientesQuery, [id]);
    ...
    `;
    const response: QueryResult = await client.query(updateQuery, [nuevoEstado, id]);
