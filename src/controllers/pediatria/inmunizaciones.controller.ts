...
export class InmunizacionesController {
  ...
  static async crear(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      ResponseHelper.created(res, 'Esquema de inmunizaciones creado exitosamente', result.rows[0]);

    } catch (error) {
      ResponseHelper.error(res, 'Error al crear esquema de inmunizaciones', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  static async obtenerEsquemaCompleto(req: Request, res: Response): Promise<void> {
    const client = await pool.connect();
    ...
    try {
      ...
      if (result.rows.length === 0) {
        ResponseHelper.notFound(res, 'Esquema de vacunación');
        return;
      }

      ResponseHelper.success(res, 'Esquema de vacunación obtenido exitosamente', result.rows[0]);

    } catch (error) {
      ResponseHelper.error(res, 'Error al obtener esquema de vacunación', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  static async obtenerHistorialCompleto(req: Request, res: Response): Promise<void> {
    const client = await pool.connect();
    ...
    try {
      ...
      ResponseHelper.success(res, 'Historial de vacunas obtenido exitosamente', result.rows);

    } catch (error) {
      ResponseHelper.error(res, 'Error al obtener historial de vacunas', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  static async actualizar(req: Request, res: Response): Promise<void> {
    ...
    try {
     ...
     if (result.rows.length === 0) {
       ResponseHelper.notFound(res, 'Esquema de inmunizaciones');
       return;
     }

     ResponseHelper.updated(res, 'Esquema de inmunizaciones actualizado exitosamente', result.rows[0]);

   } catch (error) {
     ResponseHelper.error(res, 'Error al actualizar esquema de inmunizaciones', 500, error);
   } finally {
     client.release();
   }
 }
 ...
 static async eliminar(req: Request, res: Response): Promise<void> {
   ...
   try {
     ...
     ResponseHelper.deleted(res, 'Esquema de inmunizaciones');

   } catch (error) {
     ResponseHelper.error(res, 'Error al eliminar esquema de inmunizaciones', 500, error);
   } finally {
     client.release();
   }
 }
 ...
 static async verificarCompletitud(req: Request, res: Response): Promise<void> {
   const client = await pool.connect();
   ...
   try {
     ...
     if (edadMeses >= 2) {
       ...
       if (!data.bcg_fecha) recomendaciones.push('BCG pendiente');
     }
     
     if (edadMeses >= 6) {
       esquemaEsperado += 1; // Hepatitis B completa
       if (!data.hepatitis_b_3_fecha) recomendaciones.push('Hepatitis B incompleta');
     }
     
     if (edadMeses >= 6) {
       esquemaEsperado += 1; // Pentavalente completa
       if (!data.pentavalente_3_fecha) recomendaciones.push('Pentavalente incompleta');
     }
     
     if (edadMeses >= 12) {
       esquemaEsperado += 1; // SRP
       if (!data.srp_12_meses_fecha) recomendaciones.push('SRP de 12 meses pendiente');
     }

     const porcentajeCompletitud = esquemaEsperado > 0 ? 
       Math.round((data.vacunas_basicas_aplicadas / esquemaEsperado) * 100) : 0;

     ResponseHelper.success(res, 'Verificación de completitud realizada exitosamente', {
       ...data,
     ...
     ResponseHelper.error(res, 'Error al verificar completitud del esquema', 500, error);
   } finally {
     client.release();
   }
 }
 ...
 static async obtenerProximasVacunas(req: Request, res: Response): Promise<void> {
   const client = await pool.connect();
   ...
   try {
     ...
     ResponseHelper.success(res, 'Próximas vacunas obtenidas exitosamente', {
     ...
     ResponseHelper.error(res, 'Error al obtener próximas vacunas', 500, error);
   } finally {
     client.release();
   }
 }
