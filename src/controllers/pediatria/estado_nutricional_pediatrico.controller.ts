...
export class EstadoNutricionalPediatricoController {
  ...
  static async crear(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      ResponseHelper.created(res, 'Estado nutricional creado exitosamente', result.rows[0]);

    } catch (error) {
      ResponseHelper.error(res, 'Error al crear estado nutricional', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  static async obtenerPorHistoriaClinica(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (result.rows.length === 0) {
        ResponseHelper.notFound(res, 'Estado nutricional');
        return;
      }

      ResponseHelper.success(res, 'Estado nutricional obtenido exitosamente', result.rows);

    } catch (error) {
      ResponseHelper.error(res, 'Error al obtener estado nutricional', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  static async actualizar(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      ResponseHelper.updated(res, 'Estado nutricional actualizado exitosamente', result.rows[0]);

    } catch (error) {
      ResponseHelper.error(res, 'Error al actualizar estado nutricional', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  static async eliminar(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      ResponseHelper.deleted(res, 'Estado nutricional');

    } catch (error) {
      ResponseHelper.error(res, 'Error al eliminar estado nutricional', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  static async obtenerAlertasNutricionales(req: Request, res: Response): Promise<void> {
    const client = await pool.connect();
    ...
    try {
      ...
      ResponseHelper.success(res, 'Alertas nutricionales obtenidas exitosamente', result.rows);

    } catch (error) {
      ResponseHelper.error(res, 'Error al obtener alertas nutricionales', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  static async calcularPercentiles(req: Request, res: Response): Promise<void> {
    const client = await pool.connect();
    ...
    try {
      ...
      ResponseHelper.success(res, 'Percentiles calculados exitosamente', percentiles);

    } catch (error) {
      ResponseHelper.error(res, 'Error al calcular percentiles', 500, error);
    } finally {
      client.release();
    }
  }
  ...
  private static calcularPercentilPeso(peso: number, edad_meses: number, sexo: string): number {
    // Implementación simplificada - en producción usarías tablas OMS
    if (peso < 3) return 3;
    if (peso > 20) return 97;
    return Math.floor(Math.random() * 94) + 3; // Placeholder
  }

  private static calcularPercentilTalla(talla: number, edad_meses: number, sexo: string): number {
    // Implementación simplificada
    if (talla < 50) return 3;
    if (talla > 120) return 97;
    return Math.floor(Math.random() * 94) + 3; // Placeholder
  }

  private static clasificarEstadoNutricional(peso: number, talla: number, edad_meses: number, sexo: string): string {
    const imc = peso / Math.pow(talla/100, 2);
    
    if (imc < 15) return 'Desnutrición severa';
    if (imc < 17) return 'Desnutrición moderada';
