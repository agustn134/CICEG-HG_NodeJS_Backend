...
export const getNotasPostoperatoria = async (req: Request, res: Response): Promise<Response> => {
  try {
    const response: QueryResult = await pool.query("SELECT * FROM nota_postoperatoria");
...
export const getNotaPostoperatoriaById = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const response: QueryResult = await pool.query("SELECT * FROM nota_postoperatoria WHERE id_nota_postoperatoria = $1", [id]);
...
export const deleteNotaPostoperatoria = async (req: Request, res: Response): Promise<Response> => {
  try {
    const id = req.params.id;
    const response: QueryResult = await pool.query("DELETE FROM nota_postoperatoria WHERE id_nota_postoperatoria = $1", [id]);
