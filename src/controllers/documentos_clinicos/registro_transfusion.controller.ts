...
interface RegistroTransfusion {
  id_transfusion?: number;
  id_documento: number;
...
export const getRegistrosTransfusion = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { 
      ...
      fecha_inicio,
      fecha_fin,
      con_reacciones_adversas = 'all' // 'si', 'no', 'all'
    } = req.query;
...
export const createRegistroTransfusion = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const unitQuery = await client.query(
      'SELECT id_transfusion FROM registro_transfusion WHERE numero_unidad = $1',
      [numero_unidad]
    );
...
export const updateRegistroTransfusion = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const { id } = req.params;
    const updateData: Partial<RegistroTransfusion> = req.body;

    // Verificar que el registro existe
    const existsQuery = await client.query(
      'SELECT id_transfusion FROM registro_transfusion WHERE id_transfusion = $1',
      [id]
    );
    ...
    const fields = Object.keys(updateData).filter(key => updateData[key as keyof RegistroTransfusion] !== undefined);
    const values = fields.map(field => updateData[field as keyof RegistroTransfusion]);
    ...
    if (updateData.numero_unidad) {
      const unitQuery = await client.query(
        'SELECT id_transfusion FROM registro_transfusion WHERE numero_unidad = $1 AND id_transfusion != $2',
        [updateData.numero_unidad, id]
      );
    ...
    if (updateData.id_medico_responsable) {
      const medicoQuery = await client.query(
        'SELECT id_personal_medico FROM personal_medico WHERE id_personal_medico = $1 AND activo = true',
        [updateData.id_medico_responsable]
      );
...
export const deleteRegistroTransfusion = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const response: QueryResult = await pool.query(
      "DELETE FROM registro_transfusion WHERE id_transfusion = $1 RETURNING *",
      [id]
    );
...
export const getRegistrosTransfusionByExpediente = async (req: Request, res: Response): Promise<Response> => {
  try {
...
export const getRegistrosTransfusionByPaciente = async (req: Request, res: Response): Promise<Response> => {
  try {
...
export const searchRegistrosTransfusion = async (req: Request, res: Response): Promise<Response> => {
  try {
...
export const getReaccionesAdversas = async (req: Request, res: Response): Promise<Response> => {
  try {
...
export const estadisticasTransfusiones = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const [porComponenteResult, porGrupoResult, porMesResult, medicosResult] = await Promise.all([
      pool.query(porComponenteQuery),
      pool.query(porGrupoQuery),
...
export const getCompatibilidadSanguinea = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const compatibilidades: { [key: string]: string[] } = {
      'O-': ['O-'],
      'O+': ['O-', 'O+'],
      'A-': ['O-', 'A-'],
      'A+': ['O-', 'O+', 'A-', 'A+'],
    ...
    const response: QueryResult = await pool.query(disponibilidadQuery, [gruposCompatibles]);
...
export const finalizarTransfusion = async (req: Request, res: Response): Promise<Response> => {
  const client = await pool.connect();
  ...
  try {
    ...
    const transfusionQuery = await client.query(
      'SELECT id_transfusion, fecha_inicio, fecha_fin FROM registro_transfusion WHERE id_transfusion = $1',
      [id]
    );
...
export const getTiposComponentesDisponibles = async (req: Request, res: Response): Promise<Response> => {
  try {
    return res.status(200).json({
      success: true,
      data: {
...
export const validarRegistroTransfusion = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { id } = req.params;
