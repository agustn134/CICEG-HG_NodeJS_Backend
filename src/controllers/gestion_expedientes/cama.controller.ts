// src/controllers/gestion_expedientes/cama.controller.ts
import { Request, Response } from 'express';
import { QueryResult } from 'pg';
import pool from '../../config/database';

export const getCamas = async (req: Request, res: Response): Promise<Response> => {
  try {
    const response: QueryResult = await pool.query("SELECT * FROM cama");
    return res.status(200).json(response.rows);
  } catch (error) {
    console.error(error);
    return res.status(500).send("Error al obtener camas");
  }
};

export const getCamaById = async (req: Request, res: Response): Promise<Response> => {
  try {
    const id = req.params.id;
    const response: QueryResult = await pool.query("SELECT * FROM cama WHERE id_cama = $1", [id]);
    if (response.rows.length === 0) {
      return res.status(404).send("Cama no encontrada");
    }
    return res.status(200).json(response.rows[0]);
  } catch (error) {
    console.error(error);
    return res.status(500).send("Error al obtener cama por ID");
  }
};

export const createCama = async (req: Request, res: Response): Promise<Response> => {
  try {
    const {
      numero,
      id_servicio,
      estado,
      descripcion,
      area,
      subarea
    } = req.body;
    const response: QueryResult = await pool.query(
      "INSERT INTO cama (numero, id_servicio, estado, descripcion, area, subarea) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *",
      [numero, id_servicio, estado, descripcion, area, subarea]
    );
    return res.status(201).json(response.rows[0]);
  } catch (error) {
    console.error(error);
    return res.status(500).send("Error al crear cama");
  }
};

export const updateCama = async (req: Request, res: Response): Promise<Response> => {
  try {
    const id = req.params.id;
    const {
      numero,
      id_servicio,
      estado,
      descripcion,
      area,
      subarea
    } = req.body;
    const response: QueryResult = await pool.query(
      "UPDATE cama SET numero = $1, id_servicio = $2, estado = $3, descripcion = $4, area = $5, subarea = $6 WHERE id_cama = $7 RETURNING *",
      [numero, id_servicio, estado, descripcion, area, subarea, id]
    );
    if (response.rows.length === 0) {
      return res.status(404).send("Cama no encontrada");
    }
    return res.status(200).json(response.rows[0]);
  } catch (error) {
    console.error(error);
    return res.status(500).send("Error al actualizar cama");
  }
};

export const deleteCama = async (req: Request, res: Response): Promise<Response> => {
  try {
    const id = req.params.id;
    const response: QueryResult = await pool.query("DELETE FROM cama WHERE id_cama = $1", [id]);
    if (response.rowCount === 0) {
      return res.status(404).send("Cama no encontrada");
    }
    return res.status(200).send("Cama eliminada correctamente");
  } catch (error) {
    console.error(error);
    return res.status(500).send("Error al eliminar cama");
  }
};