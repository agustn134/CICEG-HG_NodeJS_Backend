...
import { SignosVitales, GraficoSignosVitales, Dataset } from '../../types/interfaces/SignosVitales.interface';

// ==========================================
// OBTENER TODOS LOS SIGNOS VITALES
// ==========================================
export const getSignosVitales = async (req: Request, res: Response): Promise<void> => {
  try {
    const { 
      id_expediente,
      id_internamiento,
...
// ==========================================
export const getSignosVitalesById = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
...
// ==========================================
export const createSignosVitales = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (id_internamiento) {
      ...
      `;
      const internamientoResponse: QueryResult = await client.query(internamientoQuery, [id_internamiento, id_expediente]);
    ...
    const insertSignosQuery = `
      ...
      RETURNING *
    `;
    const signosResponse: QueryResult = await client.query(insertSignosQuery, [
      id_documento,
      fecha_toma,
      temperatura || null,
...
// ==========================================
export const updateSignosVitales = async (req: Request, res: Response): Promise<void> => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const { id } = req.params;
    ...
    const signosActualesQuery = `
      ...
      WHERE sv.id_signos_vitales = $1
    `;
    const signosActualesResponse: QueryResult = await client.query(signosActualesQuery, [id]);
...
// ==========================================
export const deleteSignosVitales = async (req: Request, res: Response): Promise<void> => {
  const client = await pool.connect();
  try {
    await client.query('BEGIN');
    const { id } = req.params;
    ...
    const signosQuery = `
      ...
      WHERE sv.id_signos_vitales = $1
    `;
    const signosResponse: QueryResult = await client.query(signosQuery, [id]);
    if (signosResponse.rows.length === 0) {
      await client.query('ROLLBACK');
       res.status(404).json({
        success: false,
...
// ==========================================
export const getUltimosSignosVitalesPaciente = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const query = `
      ...
      LIMIT $2
    `;
    const response: QueryResult = await pool.query(query, [id_expediente, parseInt(limite as string)]);
    if (response.rows.length === 0) {
       res.status(404).json({
        success: false,
    ...
    const data = parseInt(limite as string) === 1 ? response.rows[0] : response.rows;
     res.status(200).json({
      success: true,
...
// OBTENER HISTORIAL DE SIGNOS VITALES
// ==========================================
export const getHistorialSignosVitales = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { id_expediente } = req.params;
    const { 
      fecha_inicio, 
...
export const getGraficaSignosVitales = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { id_expediente } = req.params;
    const { 
      tipo_signo = 'todos',
    ...
    let query = `
      ...
      WHERE dc.id_expediente = $1 
        ...
        AND sv.fecha_toma >= CURRENT_DATE - INTERVAL '${parseInt(dias as string)} days'
    `;
    ...
    const datosGrafica: GraficoSignosVitales = {
  ...
  datasets: []
    ...
    return res.status(200).json({
      ...
      periodo: {
        ...
        desde: new Date(Date.now() - parseInt(dias as string) * 24 * 60 * 60 * 1000),
        hasta: new Date()
      }
    });
  } catch (error) {
