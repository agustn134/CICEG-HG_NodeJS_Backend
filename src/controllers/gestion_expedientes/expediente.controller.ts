...
export const getExpedientes = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const response: QueryResult = await client.query(query, params); // Usar client en lugar de pool
...
export const deleteExpediente = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const existeResponse: QueryResult = await client.query(existeQuery, [id]);
    ...
    const usoResponse: QueryResult = await client.query(usoQuery, [id]);
    ...
    if ((totalDocumentos > 0 || totalInternamientos > 0) && force !== true) {
      ...
      const response: QueryResult = await client.query(updateQuery, [id, motivo_eliminacion]);
...
// ==========================================
export const getExpedientesByPaciente = async (req: Request, res: Response): Promise<Response> => {
  try {
...
// ==========================================
export const buscarExpedientes = async (req: Request, res: Response): Promise<Response> => {
  try {
...
// ==========================================
export const getDashboardExpedientes = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const internamientosActivosResponse: QueryResult = await pool.query(internamientosActivosQuery);
    ...
    const masActivosResponse: QueryResult = await pool.query(masActivosQuery);
    ...
    const alertasResponse: QueryResult = await pool.query(alertasQuery);
...
export const getExpedienteById = async (req: Request, res: Response): Promise<Response> => {
  try {
    ...
    const validacionResponse: QueryResult = await pool.query(validacionReingresoQuery, [id, 1]);
...
export const createExpediente = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const pacienteExisteResponse: QueryResult = await client.query(pacienteExisteQuery, [id_paciente]);
    ...
    if (!numeroExpedienteFinal) {
      ...
      const lastExpedienteResponse: QueryResult = await client.query(lastExpedienteQuery);
    ...
    const existeExpedienteResponse: QueryResult = await client.query(existeExpedienteQuery, [numeroExpedienteFinal]);
    ...
    if (crear_historia_clinica && id_medico_creador) {
      ...
      const medicoExisteResponse: QueryResult = await client.query(medicoExisteQuery, [id_medico_creador]);
      ...
      if (medicoExisteResponse.rows.length > 0) {
        ...
        if (tipoDocumentoResponse.rows.length > 0) {
          ...
          const historiaResponse: QueryResult = await client.query(insertHistoriaQuery, [
...
export const updateExpediente = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    const expedienteActualResponse: QueryResult = await client.query(expedienteActualQuery, [id]);
    ...
    if (estado && estado !== expedienteActual.estado) {
      ...
      if (estado === 'Cerrado') {
        ...
        const internamientosActivosResponse: QueryResult = await client.query(internamientosActivosQuery, [id]);
...
// ==========================================
export const getAuditoriaExpediente = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { id } = req.params;
    const { 
      fecha_inicio, 
    ...
    const expedienteExisteResponse: QueryResult = await pool.query(expedienteExisteQuery, [id]);
...
// ==========================================
export const getAlertasExpediente = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { id } = req.params;
    ...
    const alertasPorTipo = {
      CRITICA: response.rows.filter((a: any) => a.tipo_alerta === 'CRITICA'),
      ADVERTENCIA: response.rows.filter((a: any) => a.tipo_alerta === 'ADVERTENCIA'),
      INFORMATIVA: response.rows.filter((a: any) => a.tipo_alerta === 'INFORMATIVA')
    };
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        resumen: {
          total: response.rowCount,
          activas: response.rows.filter((a: any) => a.estado === 'ACTIVA').length,
          revisadas: response.rows.filter((a: any) => a.estado === 'REVISADA').length,
          cerradas: response.rows.filter((a: any) => a.estado === 'CERRADA').length
        }
      }
    });
  } catch (error) {
...
// ==========================================
export const updateAlertaExpediente = async (req: Request, res: Response): Promise<Response> => {
  const client = await pool.connect();
  ...
  try {
    ...
    const alertaExisteResponse: QueryResult = await client.query(alertaExisteQuery, [id_alerta, id]);
...
// ==========================================
export const generarReporteExpediente = async (req: Request, res: Response): Promise<Response> => {
  try {
    const { id } = req.params;
...
// ==========================================
export const validarReingresoExpediente = async (req: Request, res: Response): Promise<Response> => {
  const client = await pool.connect();
  ...
  try {
    ...
    const validacionResponse: QueryResult = await client.query(insertValidacionQuery, [
      id,
...
export const getExpedienteByPacienteId = async (req: Request, res: Response): Promise<void> => {
  try {
