...
-- ==========================================
-- TABLA INMUNIZACIONES - VERSIÓN FINAL
-- Hospital General San Luis de la Paz
...
-- ==========================================
-- ESTRUCTURA COMPLETA DEL SISTEMA DE INMUNIZACIONES
-- ==========================================

/*
AHORA TIENES 3 TABLAS TRABAJANDO JUNTAS:

1. ┌─────────────────────────────────────────────────────────────┐
   │                   INMUNIZACIONES                            │
   │  (Esquema básico - columnas fijas)                          │
   │  - BCG, Hepatitis B, Pentavalente, Rotavirus, etc.         │
   │  - Fácil de llenar, rápido de consultar                     │
   └─────────────────────────────────────────────────────────────┘
                                   │
                                   │ (se relaciona con)
                                   ▼
   ┌─────────────────────────────────────────────────────────────┐
   │                VACUNAS_ADICIONALES                          │
   │  (Vacunas extra que no están en el esquema básico)         │
   │  - COVID-19, Meningocócica, vacunas nuevas, etc.           │
   │  - Flexibilidad total                                       │
   └─────────────────────────────────────────────────────────────┘
                                   │
                                   │ (puede usar)
   ...
   ┌─────────────────────────────────────────────────────────────┐
   │                CATALOGO_VACUNAS                             │
   │  (Vacunas predefinidas comunes adicionales)                │
   │  - Para no escribir siempre el mismo nombre                │
   │  - Se pueden agregar nuevas al catálogo                     │
   └─────────────────────────────────────────────────────────────┘

*/

-- ==========================================
-- MEJORA SISTEMA DE INMUNIZACIONES
-- Hospital General San Luis de la Paz
-- Agregar flexibilidad para vacunas adicionales
-- ==========================================

-- ==========================================
-- PASO 1: CREAR CATÁLOGO DE VACUNAS
-- ==========================================
...
/*
-- Ejemplo 1: Agregar vacuna que está en el catálogo
SELECT agregar_vacuna_adicional(
  123, -- id_inmunizacion
  'COVID-19 Pfizer', -- nombre_vacuna (está en catálogo)
  '2024-06-15', -- fecha_aplicacion
  1, -- primera dosis
  'ABC123', -- lote
  'Pfizer', -- laboratorio
  'Intramuscular', -- vía
  'Brazo izquierdo', -- sitio
  'Enfermera María García', -- aplicada_por
  'Hospital General San Luis de la Paz', -- institución
  NULL, -- sin reacciones
  'Primera dosis COVID-19', -- observaciones
  45 -- registrado_por (id_medico)
);

-- Ejemplo 2: Agregar vacuna que NO está en el catálogo (escribir libremente)
SELECT agregar_vacuna_adicional(
  123, -- id_inmunizacion
  'Vacuna Experimental XYZ', -- nombre_vacuna (NO está en catálogo)
  '2024-06-20', -- fecha_aplicacion
  1, -- dosis
  'EXP456', -- lote
  'Laboratorio Experimental', -- laboratorio
  'Subcutánea', -- vía
  'Muslo derecho', -- sitio
  'Dr. Juan Pérez', -- aplicada_por
  'Centro de Investigación', -- institución
  'Leve enrojecimiento local', -- reacciones
  'Vacuna experimental autorizada', -- observaciones
  45 -- registrado_por
);

-- Ejemplo 3: Ver historial completo de vacunas de un paciente
SELECT * FROM obtener_historial_vacunas_completo(123);

-- Ejemplo 4: Ver solo las vacunas adicionales
SELECT * FROM detalle_vacunas_adicionales WHERE id_inmunizacion = 123;
*/
...
-- SELECT 'Sistema de inmunizaciones mejorado - Ahora se pueden agregar vacunas adicionales dinámicamente' as resultado;
...
/*
-- Ejemplo de uso para crear una nota de evolución:

SELECT crear_nota_evolucion_con_plantilla(
  123, -- id_expediente
  45,  -- id_medico
  'Paciente tranquila, con apetito, sin vómito, tolerancia oral adecuada...', -- sintomas_signos
  'Paciente femenino, caquéctica, facies marásmica...', -- habitus_exterior
  'Peso bajo y talla baja para la edad', -- estado_nutricional
  'UROANALISIS: sin estudios recientes que comentar', -- estudios_lab
  'Paciente con desnutrición severa, con remisión gradual...', -- evolucion_analisis
  'E41 MARASMO NUTRICIONAL, B370 ESTOMATITIS CANDIDIASICA...', -- diagnosticos
  'Estabilización, dieta guiada por nutrición...', -- plan_tratamiento
  'Reservado a evolución clínica', -- pronostico
  'Interconsulta con nutrición solicitada', -- interconsultas (opcional)
  'Dieta suave fraccionada, soluciones parenterales...' -- indicaciones (opcional)
);
*/
...
-- SELECT 'Tabla nota_evolucion actualizada exitosamente con formato específico del hospital' as resultado;
