...
exports.updateEstadoTraslado = exports.deleteReferenciaTraslado = exports.updateReferenciaTraslado = exports.createReferenciaTraslado = exports.getReferenciaTrasladoById = exports.getReferenciasTraslado = void 0;
...
const getReferenciasTraslado = async (req, res) => {
    try {
        ...
        // Filtros opcionales
        const tipoTraslado = req.query.tipo_traslado;
...
const updateReferenciaTraslado = async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const id = req.params.id;
        const { establecimiento_origen, establecimiento_destino, motivo_envio, resumen_clinico, diagnostico, id_guia_diagnostico, plan_tratamiento, estado_fisico, pronostico, tipo_traslado, medico_receptor, observaciones } = req.body;
...
const updateEstadoTraslado = async (req, res) => {
    const client = await database_1.default.connect();
    try {
        await client.query('BEGIN');
        const id = req.params.id;
        ...
        // Verificar que la referencia existe
        const checkQuery = await client.query('SELECT id_referencia FROM referencia_traslado WHERE id_referencia = $1', [id]);
        if (checkQuery.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(404).send("Referencia de traslado no encontrada");
        }
...
exports.updateEstadoTraslado = updateEstadoTraslado;
