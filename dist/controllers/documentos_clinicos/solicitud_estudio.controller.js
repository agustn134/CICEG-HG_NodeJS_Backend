...
exports.updateResultadoEstudio = exports.deleteSolicitudEstudio = exports.updateSolicitudEstudio = exports.createSolicitudEstudio = exports.getSolicitudEstudioById = exports.getSolicitudesEstudio = void 0;
...
const createSolicitudEstudio = async (req, res) => {
    ...
    try {
        ...
        const { id_documento, id_estudio, justificacion, preparacion_requerida, informacion_clinica_relevante, fecha_solicitada, prioridad = 'Normal', fecha_realizacion = null, hora_toma_muestra = null, resultado = null, interpretacion = null, estado = 'Solicitado' } = req.body;
...
const updateSolicitudEstudio = async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const id = req.params.id;
        const { id_estudio, justificacion, preparacion_requerida, informacion_clinica_relevante, prioridad, fecha_realizacion, hora_toma_muestra, resultado, interpretacion, estado } = req.body;
      ...
      RETURNING *
    `;
        const response = await client.query(updateQuery, params);
        await client.query('COMMIT');
        return res.status(200).json(response.rows[0]);
    }
    catch (error) {
...
const updateResultadoEstudio = async (req, res) => {
    const client = await database_1.default.connect();
    try {
        await client.query('BEGIN');
        const id = req.params.id;
        ...
        const checkQuery = await client.query('SELECT estado FROM solicitud_estudio WHERE id_solicitud = $1', [id]);
        if (checkQuery.rows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(404).send("Solicitud de estudio no encontrada");
        }
...
exports.updateResultadoEstudio = updateResultadoEstudio;
