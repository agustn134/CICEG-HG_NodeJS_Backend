...
exports.DashboardPediatricoController = void 0;
const database_1 = __importDefault(require("../../config/database"));
const responses_1 = require("../../utils/responses");
class DashboardPediatricoController {
    ...
    static async obtenerEstadisticas(req, res) {
      ...
      `;
            ...
            responses_1.ResponseHelper.statistics(res, 'Estadísticas Pediátricas', result.rows[0]);
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al obtener estadísticas pediátricas', 500, error);
        }
        finally {
            client.release();
        }
    ...
    static async obtenerPacientesActivos(req, res) {
        const client = await database_1.default.connect();
        try {
      ...
      `;
            ...
            responses_1.ResponseHelper.success(res, 'Pacientes pediátricos activos obtenidos exitosamente', result.rows);
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al obtener pacientes pediátricos activos', 500, error);
        }
        finally {
            client.release();
        }
    ...
    static async obtenerDistribucionEdades(req, res) {
        const client = await database_1.default.connect();
        try {
            const query = `
        SELECT 
      ...
      `;
            const result = await client.query(query);
            responses_1.ResponseHelper.success(res, 'Distribución por edades obtenida exitosamente', result.rows);
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al obtener distribución de edades', 500, error);
        }
        finally {
            client.release();
        }
    ...
    static async obtenerAlertasPediatricas(req, res) {
        const client = await database_1.default.connect();
        try {
            const query = `
        SELECT 
      ...
      `;
            const result = await client.query(query);
            responses_1.ResponseHelper.success(res, 'Alertas pediátricas obtenidas exitosamente', result.rows);
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al obtener alertas pediátricas', 500, error);
        }
        finally {
            client.release();
        }
...
exports.DashboardPediatricoController = DashboardPediatricoController;
