...
exports.DesarrolloPsicomotrizController = void 0;
const database_1 = __importDefault(require("../../config/database"));
const responses_1 = require("../../utils/responses");
class DesarrolloPsicomotrizController {
    // Crear desarrollo psicomotriz
    static async crear(req, res) {
        const client = await database_1.default.connect();
        try {
            const { id_historia_clinica, sostuvo_cabeza_meses, se_sento_meses, gateo_meses, camino_meses, primera_palabra_meses, primeras_frases_meses, sonrisa_social_meses, reconocimiento_padres_meses, control_diurno_meses, control_nocturno_meses, juego_simbolico_meses, seguimiento_instrucciones_meses, desarrollo_normal, observaciones_desarrollo, necesita_estimulacion, tipo_estimulacion } = req.body;
            const query = `
      ...
      `;
            ...
            responses_1.ResponseHelper.created(res, 'Desarrollo psicomotriz creado exitosamente', result.rows[0]);
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al crear desarrollo psicomotriz', 500, error);
        }
        finally {
            client.release();
        }
    ...
    static async obtenerPorHistoriaClinica(req, res) {
      ...
      `;
            ...
            if (result.rows.length === 0) {
                responses_1.ResponseHelper.notFound(res, 'Desarrollo psicomotriz');
                return;
            }
            responses_1.ResponseHelper.success(res, 'Desarrollo psicomotriz obtenido exitosamente', result.rows[0]);
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al obtener desarrollo psicomotriz', 500, error);
        }
        finally {
            client.release();
        }
    ...
    static async actualizar(req, res) {
        ...
        try {
            const { id } = req.params;
            const { sostuvo_cabeza_meses, se_sento_meses, gateo_meses, camino_meses, primera_palabra_meses, primeras_frases_meses, sonrisa_social_meses, reconocimiento_padres_meses, control_diurno_meses, control_nocturno_meses, juego_simbolico_meses, seguimiento_instrucciones_meses, desarrollo_normal, observaciones_desarrollo, necesita_estimulacion, tipo_estimulacion } = req.body;
            const query = `
      ...
      `;
            ...
            responses_1.ResponseHelper.updated(res, 'Desarrollo psicomotriz actualizado exitosamente', result.rows[0]);
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al actualizar desarrollo psicomotriz', 500, error);
        }
        finally {
            client.release();
        }
    ...
    static async eliminar(req, res) {
      ...
      `;
            ...
            responses_1.ResponseHelper.deleted(res, 'Desarrollo psicomotriz');
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al eliminar desarrollo psicomotriz', 500, error);
        }
        finally {
            client.release();
        }
    ...
    static async obtenerHitosPorEdad(req, res) {
        const client = await database_1.default.connect();
        try {
        ...
        SELECT 
          ...
          '10-14 meses'
      `;
            const result = await client.query(query, [edad_meses]);
            responses_1.ResponseHelper.success(res, 'Hitos del desarrollo obtenidos exitosamente', result.rows);
        }
        catch (error) {
            responses_1.ResponseHelper.error(res, 'Error al obtener hitos por edad', 500, error);
        }
        finally {
            client.release();
        }
...
exports.DesarrolloPsicomotrizController = DesarrolloPsicomotrizController;
