...
exports.deleteAreaInterconsulta = exports.updateAreaInterconsulta = exports.createAreaInterconsulta = exports.getAreaInterconsultaById = exports.getAreasInterconsulta = void 0;
...
const createAreaInterconsulta = async (req, res) => {
    try {
        ...
        const response = await database_1.default.query("INSERT INTO area_interconsulta (nombre, descripcion, activo) VALUES ($1, $2, $3) RETURNING *", [nombre, descripcion, activo]);
...
const updateAreaInterconsulta = async (req, res) => {
    try {
        const id = req.params.id;
        const { nombre, descripcion, activo } = req.body;
        const response = await database_1.default.query("UPDATE area_interconsulta SET nombre = $1, descripcion = $2, activo = $3 WHERE id_area_interconsulta = $4 RETURNING *", [nombre, descripcion, activo, id]);
        if (response.rows.length === 0) {
            return res.status(404).send("Área de interconsulta no encontrada");
        }
...
const deleteAreaInterconsulta = async (req, res) => {
    try {
        const id = req.params.id;
        const response = await database_1.default.query("DELETE FROM area_interconsulta WHERE id_area_interconsulta = $1", [id]);
