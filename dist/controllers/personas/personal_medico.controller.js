...
const updatePersonalMedico = async (req, res) => {
    try {
        const { id } = req.params;
        const { numero_cedula, especialidad, cargo, departamento, activo, foto } = req.body;
        if (!id || isNaN(parseInt(id))) {
            return res.status(400).json({
                success: false,
                message: 'El ID debe ser un número válido'
    ...
    `;
        const duplicadoResponse = await database_1.default.query(duplicadoQuery, [numero_cedula.trim(), id]);
