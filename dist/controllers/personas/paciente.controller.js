...
exports.getHistorialMedicoResumido = exports.getEstadisticasPacientes = exports.buscarPacientes = exports.deletePaciente = exports.updatePaciente = exports.createPaciente = exports.getPacienteById = exports.getPacientes = void 0;
...
const getPacientes = async (req, res) => {
    ...
    `;
        ...
        if (edad_min) {
            query += ` HAVING DATE_PART('year', AGE(p.fecha_nacimiento)) >= ${parseInt(edad_min)}`;
        }
        if (edad_max) {
            const connector = edad_min ? ' AND' : ' HAVING';
            query += `${connector} DATE_PART('year', AGE(p.fecha_nacimiento)) <= ${parseInt(edad_max)}`;
        }
...
const createPaciente = async (req, res) => {
    const client = await database_1.default.connect(); // 🔧 Usar transacción
    try {
        await client.query('BEGIN'); // 🔧 Iniciar transacción
        const { id_persona, tipo_sangre_id, // 🔧 AGREGAR tipo_sangre_id
        alergias, transfusiones = false, detalles_transfusiones, familiar_responsable, parentesco_familiar, telefono_familiar, ocupacion, escolaridad, lugar_nacimiento } = req.body;
        // Validaciones básicas
        if (!id_persona) {
...
const updatePaciente = async (req, res) => {
    try {
        const { id } = req.params;
        const { alergias, transfusiones, detalles_transfusiones, familiar_responsable, parentesco_familiar, telefono_familiar, ocupacion, escolaridad, lugar_nacimiento } = req.body;
        if (!id || isNaN(parseInt(id))) {
            return res.status(400).json({
                success: false,
                message: 'El ID debe ser un número válido'
...
// ==========================================
const getEstadisticasPacientes = async (req, res) => {
    try {
...
`;
        const edadResponse = await database_1.default.query(edadQuery);
    ...
    `;
        const tipoSangreResponse = await database_1.default.query(tipoSangreQuery);
      ...
      LIMIT 10
    `;
        const masExpedientesResponse = await database_1.default.query(masExpedientesQuery);
        return res.status(200).json({
            success: true,
...
exports.getEstadisticasPacientes = getEstadisticasPacientes;
// ==========================================
// OBTENER HISTORIAL MÉDICO RESUMIDO
// ==========================================
const getHistorialMedicoResumido = async (req, res) => {
    try {
        const { id } = req.params;
        if (!id || isNaN(parseInt(id))) {
            return res.status(400).json({
      ...
      WHERE pac.id_paciente = $1
    `;
        const pacienteResponse = await database_1.default.query(pacienteQuery, [id]);
      ...
      ORDER BY e.fecha_creacion DESC
    `;
        const historialResponse = await database_1.default.query(historialQuery, [id]);
...
exports.getHistorialMedicoResumido = getHistorialMedicoResumido;
