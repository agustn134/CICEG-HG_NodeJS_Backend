...
exports.getEstadisticasPersonas = exports.buscarPersonas = exports.deletePersona = exports.updatePersona = exports.createPersona = exports.getPersonaById = exports.getPersonas = void 0;
...
const getPersonas = async (req, res) => {
    ...
    `;
        ...
        let havingClause = '';
        if (edad_min) {
            havingClause += ` DATE_PART('year', AGE(p.fecha_nacimiento)) >= ${parseInt(edad_min)}`;
        }
        if (edad_max) {
            const connector = edad_min ? ' AND' : '';
            havingClause += `${connector} DATE_PART('year', AGE(p.fecha_nacimiento)) <= ${parseInt(edad_max)}`;
        }
        if (havingClause) {
...
const createPersona = async (req, res) => {
    try {
        ...
        WHERE curp = $1
      `;
            const curpExisteResponse = await database_1.default.query(curpExisteQuery, [curp.trim()]);
      ...
      `;
            const tipoSangreResponse = await database_1.default.query(tipoSangreQuery, [tipo_sangre_id]);
...
const updatePersona = async (req, res) => {
    ...
    `;
      ...
      `;
            const curpDuplicadaResponse = await database_1.default.query(curpDuplicadaQuery, [curp.trim(), id]);
...
const deletePersona = async (req, res) => {
    ...
    `;
        ...
        if (totalUso > 0) {
            const roles = [];
            if (parseInt(uso.es_paciente) > 0)
                roles.push('Paciente');
            if (parseInt(uso.es_personal_medico) > 0)
                roles.push('Personal Médico');
            if (parseInt(uso.es_administrador) > 0)
...
exports.buscarPersonas = buscarPersonas;
// ==========================================
// OBTENER ESTADÍSTICAS DE PERSONAS
// ==========================================
const getEstadisticasPersonas = async (req, res) => {
    try {
    ...
    `;
      ...
      LEFT JOIN administrador adm ON p.id_persona = adm.id_persona
    `;
        const rolesResponse = await database_1.default.query(rolesQuery);
...
exports.getEstadisticasPersonas = getEstadisticasPersonas;
