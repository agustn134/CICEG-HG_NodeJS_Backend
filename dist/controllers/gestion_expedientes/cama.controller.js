...
exports.cambiarEstadoCama = exports.getEstadisticasCamas = exports.getCamasDisponiblesByServicio = exports.deleteCama = exports.updateCama = exports.createCama = exports.liberarCama = exports.getReporteRotacionCamas = exports.getOcupacionTiempoReal = exports.getCamaById = exports.getCamas = void 0;
...
const getCamas = async (req, res) => {
    try {
        const { page = 1, limit = 20, estado, id_servicio, area, subarea, buscar, solo_disponibles } = req.query;
...
// ==========================================
const getOcupacionTiempoReal = async (req, res) => {
    try {
        const query = `
      SELECT 
    ...
    `;
        ...
        const analisis = {
            ...
            por_servicio: response.rows.reduce((acc, row) => {
                if (!acc[row.servicio]) {
                    acc[row.servicio] = {
                        total: 0,
                        ocupadas: 0,
                ...
                if (row.estado === 'Ocupada')
                    acc[row.servicio].ocupadas++;
                else if (row.estado === 'Disponible')
                    acc[row.servicio].disponibles++;
                else
                    acc[row.servicio].otros++;
                return acc;
            }, {})
        };
        return res.status(200).json({
...
exports.getOcupacionTiempoReal = getOcupacionTiempoReal;
// ==========================================
// OBTENER REPORTE DE ROTACIÓN DE CAMAS
// ==========================================
const getReporteRotacionCamas = async (req, res) => {
    try {
...
exports.getReporteRotacionCamas = getReporteRotacionCamas;
// // ==========================================
...
const liberarCama = async (req, res) => {
    try {
        ...
        // Verificar que la cama existe y está ocupada
        const camaCheck = await database_1.default.query('SELECT id_cama, numero, estado FROM cama WHERE id_cama = $1', [id]);
        if (camaCheck.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Cama no encontrada'
        ...
        // Verificar que hay un internamiento activo
        const internamientoActivo = await database_1.default.query('SELECT id_internamiento FROM internamiento WHERE id_cama = $1 AND fecha_egreso IS NULL', [id]);
        if (internamientoActivo.rows.length === 0) {
            return res.status(400).json({
                success: false,
                message: 'No hay un internamiento activo para esta cama'
        ...
        // Liberar cama (cambiar a estado de limpieza/preparación)
        const response = await database_1.default.query(`UPDATE cama 
       SET estado = 'Contaminada', 
           descripcion = $1
       WHERE id_cama = $2 
       RETURNING *`, [`Liberada por: ${motivo_liberacion}. Requiere limpieza antes del próximo uso.`, id]);
        return res.status(200).json({
            success: true,
            message: `Cama ${camaCheck.rows[0].numero} liberada correctamente. Estado cambiado a 'Contaminada' para limpieza`,
            data: response.rows[0],
...
exports.liberarCama = liberarCama;
// ==========================================
// CREAR NUEVA CAMA
// ==========================================
const createCama = async (req, res) => {
    try {
        const { numero, id_servicio, estado = 'Disponible', descripcion, area, subarea } = req.body;
        ...
        const servicioCheck = await database_1.default.query('SELECT id_servicio, nombre FROM servicio WHERE id_servicio = $1 AND activo = true', [id_servicio]);
        ...
        const camaExistente = await database_1.default.query('SELECT id_cama FROM cama WHERE numero = $1 AND id_servicio = $2', [numero.trim(), id_servicio]);
...
const updateCama = async (req, res) => {
    try {
        ...
        // Verificar que la cama existe
        const camaCheck = await database_1.default.query('SELECT id_cama, estado FROM cama WHERE id_cama = $1', [id]);
        if (camaCheck.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message: 'Cama no encontrada'
        ...
        if (updateData.estado) {
            ...
            if (updateData.estado === 'Ocupada' && estadoActual !== 'Ocupada') {
                const pacienteActual = await database_1.default.query('SELECT id_internamiento FROM internamiento WHERE id_cama = $1 AND fecha_egreso IS NULL', [id]);
                if (pacienteActual.rows.length > 0) {
                    return res.status(400).json({
                        success: false,
        ...
        if (updateData.id_servicio) {
            const servicioCheck = await database_1.default.query('SELECT id_servicio FROM servicio WHERE id_servicio = $1 AND activo = true', [updateData.id_servicio]);
            if (servicioCheck.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    message: 'El servicio especificado no existe o no está activo'
        ...
        if (updateData.numero) {
            const servicioId = updateData.id_servicio ||
                (await database_1.default.query('SELECT id_servicio FROM cama WHERE id_cama = $1', [id])).rows[0].id_servicio;
            const camaExistente = await database_1.default.query('SELECT id_cama FROM cama WHERE numero = $1 AND id_servicio = $2 AND id_cama != $3', [updateData.numero.trim(), servicioId, id]);
            if (camaExistente.rows.length > 0) {
                return res.status(409).json({
                    success: false,
...
const deleteCama = async (req, res) => {
    try {
        ...
        const internamientosActivos = await database_1.default.query('SELECT id_internamiento FROM internamiento WHERE id_cama = $1 AND fecha_egreso IS NULL', [id]);
        ...
        const tieneHistorial = await database_1.default.query('SELECT COUNT(*) as total FROM internamiento WHERE id_cama = $1', [id]);
        const totalUsos = parseInt(tieneHistorial.rows[0].total);
        if (totalUsos > 0) {
            // Si tiene historial, mejor cambiar estado a fuera de servicio
            await database_1.default.query("UPDATE cama SET estado = 'Mantenimiento', descripcion = 'Cama dada de baja - No usar' WHERE id_cama = $1", [id]);
            return res.status(200).json({
                success: true,
            ...
            await database_1.default.query('DELETE FROM cama WHERE id_cama = $1', [id]);
            return res.status(200).json({
                success: true,
...
// ==========================================
const getCamasDisponiblesByServicio = async (req, res) => {
    try {
...
exports.getCamasDisponiblesByServicio = getCamasDisponiblesByServicio;
// ==========================================
// OBTENER ESTADÍSTICAS DE CAMAS
// ==========================================
const getEstadisticasCamas = async (req, res) => {
    try {
        const queries = {
            totalCamas: `
...
exports.getEstadisticasCamas = getEstadisticasCamas;
// ==========================================
// CAMBIAR ESTADO DE CAMA
// ==========================================
const cambiarEstadoCama = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const descripcionUpdate = motivo ?
            `Estado cambiado a ${nuevo_estado}. Motivo: ${motivo}` :
            `Estado cambiado a ${nuevo_estado}`;
        const response = await database_1.default.query('UPDATE cama SET estado = $1, descripcion = $2 WHERE id_cama = $3 RETURNING *', [nuevo_estado, descripcionUpdate, id]);
        return res.status(200).json({
            success: true,
            message: `Estado de cama ${camaCheck.rows[0].numero} cambiado de ${estadoActual} a ${nuevo_estado}`,
            data: response.rows[0]
...
exports.cambiarEstadoCama = cambiarEstadoCama;
